...
const schema = z.object({
  content: z.string().min(1, "学习计划内容不能为空"),
});
...
export async function PATCH(request: Request, context: Context) {
  return requireApiUser(request, async (user) => {
    ...
    const existing = await prisma.studyPlan.findFirst({ where: { id, storeId: requireStoreId(user) } });
    if (!existing) throw new ApiError("学习计划不存在或不属于当前门店", 404);
    const record = await prisma.studyPlan.update({
      where: { id },
      data: { content },
